...
export function analyse(all:Message[],start:number,end:number,maxGap:number,night:boolean,excludeStart=23*60,excludeEnd=7*60){
 ...
 const stats=names.map(who=>{const msgs=filtered.filter(m=>m.who===who),txt=msgs.filter(m=>isText(m.text));const replies:number[]=[];const openers:Record<string,number>=Object.create(null);const closers:Record<string,number>=Object.create(null);const openerExamples:Record<string,string>=Object.create(null);const closerExamples:Record<string,string>=Object.create(null);let starts=0;
 turns.forEach((t,i)=>{const next=turns[i+1];if(t.who===who&&t.last>=start&&t.last<=end&&next&&next.first-t.last>=6*3600000&&isText(t.lastText)){const word=t.lastText.match(/[\p{L}\p{N}]+(?:[’'][\p{L}\p{N}]+)*/gu)?.at(-1)?.toLocaleLowerCase();if(word){closers[word]=(closers[word]||0)+1;closerExamples[word]??=t.lastText;}}if(t.who!==who||t.first<start||t.first>end)return;const p=turns[i-1];if(!p||t.first-p.last>=6*3600000){starts++;if(isText(t.text)){const word=t.text.match(/[\p{L}\p{N}]+(?:[’'][\p{L}\p{N}]+)*/u)?.[0].toLocaleLowerCase();if(word){openers[word]=(openers[word]||0)+1;openerExamples[word]??=t.text;}}}if(!p||p.who===who||p.last<start)return;const gap=(t.first-p.last)/60000;if(gap>maxGap)return;
