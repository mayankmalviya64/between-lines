...
export function demoChat():Message[]{const out:Message[]=[];const end=new Date();end.setHours(19,0,0,0);for(let day=59;day>=0;day--){for(let j=0;j<3+(day%5);j++){const at=+end-day*86400000+j*900000;const first=(day+j)%3===0?'Alex':'Sam';out.push({at,who:first,text:j%3===0?'How was your day? I was thinking about our plans for the weekend. It would be lovely to catch up and hear everything.':'Thanks for checking in, I appreciate it!'});out.push({at:at+(2+(day*j)%25)*60000,who:first==='Alex'?'Sam':'Alex',text:j%2?'That sounds good! What time works for you?':'I am proud of you. You have been putting so much effort into this and I hope you take a little time for yourself too.'});}}return out.sort((a,b)=>a.at-b.at);}
