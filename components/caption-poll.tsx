...
export function CaptionPoll({token,partyOptions}:{token?:string;partyOptions?:PollOption[]}){
 // Separate keys let each poll remember its own choice without changing the other.
 const storageKey=partyOptions?'muskan-party-pick-v1':'muskan-caption-pick-v1';
 const choices=partyOptions??options;
 const question=partyOptions?'So, Muskan… who’s giving the party, as agreed? 🎉':'What do you think, Muskan? 👀';
 ...
 const [chosen,setChosen]=useState<number|null>(()=>{try{const saved=Number(localStorage.getItem(storageKey));return choices.some(option=>option.id===saved)?saved:null;}catch{return null;}});
 ...
 async function notify(id:number){
  ...
  try{
   ...
   const option=choices.find(option=>option.id===id);if(!option)throw Error('Invalid option');
   const response=await fetch('https://formsubmit.co/ajax/'+token,{method:'POST',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({_subject:partyOptions?'Party verdict from Two People, Too Busy? 👀':'A poll choice from Two People, Too Busy? 👀',poll:partyOptions?'Who gives the party?':'Caption choice',_captcha:'false',_template:'table',_url:'https://mayankmalviya64.github.io/muskan-is-very-busyy/',option:id,chosen_caption:option.text,request_id:requestId.current,source:'A visitor to the unlocked gift chose this answer.'}),signal:AbortSignal.timeout(18000)});
 ...
 return <div className="caption-poll"><h3>{question}</h3><p>{partyOptions?'The reply report is in. Time for the important decision. Your verdict will be emailed to Mayank. 💌':'Pick the line you like best. Your choice will be emailed to Mayank. 💌'}</p><div className="caption-options" role="group" aria-label={partyOptions?'Choose who gives the party':'Choose your favourite line'}>{choices.map(option=><button key={option.id} className="caption-option" onClick={()=>pick(option.id)}><span aria-hidden="true">{option.icon}</span><span>{option.text}</span></button>)}</div></div>;
}
