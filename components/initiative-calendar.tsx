...
export function InitiativeCalendar({chat,people,avatars}:{chat:Message[];people:{who:string}[];avatars:string[]}) {
 ...
 return <div className="initiative-calendar">
  ...
  <div className="calendar-key">{people.map((person,index)=><span key={person.who}>{avatars[index]} {person.who}</span>)}<span>· Messages, no new start</span><span>Blank: no recorded messages</span></div>
   ...
   return <section className="calendar-month" key={+month} aria-label={month.toLocaleDateString(undefined,{month:'long',year:'numeric'})}>
    ...
    <div className="calendar-grid">{weekdays.map(day=><span className="calendar-weekday" key={day}>{day}</span>)}
     ...
     {Array.from({length:count},(_,index)=>{
      ...
      const description=outside?'Outside the three-month range':starters.length?starters.map(person=>person.who+': '+person.count+' conversation '+(person.count===1?'start':'starts')).join('; '):day?'Messages, no new conversation start':'No recorded messages';
