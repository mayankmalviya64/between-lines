...
// Only these app events are sent; no chat data is attached.
// GA4 Enhanced measurement separately adds automatic interaction events after consent.
...
export function UsageConsent(){
 ...
 return <aside className="card" style={{maxWidth:1204,margin:'20px auto',padding:18}} aria-label="Usage analytics preference">{choice?<div className="inline"><small>Usage analytics: {choice==='yes'?'allowed':'off'}. You can change this at any time.</small><button className="secondary" onClick={()=>setChoice('')}>Change preference</button></div>:<><strong>Help improve Between Lines?</strong><p style={{fontSize:14,lineHeight:1.6}}>Allow Google Analytics to measure visits, feature use and automatic interactions such as scrolling and outbound clicks. It uses cookies and collects technical browser/device data and interaction metadata, such as link URLs. Imported chat text, participant names, imported filenames, relationship type and chat statistics are not included in our analytics events. Declining does not affect analysis.</p><div className="inline"><button className="primary" onClick={()=>choose(true)}>Allow usage analytics</button><button className="secondary" onClick={()=>choose(false)}>No thanks</button></div></>}</aside>;
}
export function AnalyticsSettings(){return <main className="admin"><a className="secondary" href="./">Back to insights</a><h1>Usage analytics</h1><div className="card"><h2>{analyticsConfigured?'Google Analytics connected':'Google Analytics setup prepared'}</h2><p>{analyticsConfigured?'Open your Google Analytics account to see consented visits, imports, demo usage and filter events.':'The site owner needs to connect a Google Analytics Measurement ID before collection can begin. No usage data is being collected by this deployment yet.'}</p><a className="primary" href="https://analytics.google.com/" target="_blank" rel="noreferrer">Open Google Analytics</a><div className="note">Reports are protected by your Google account. Our custom usage events contain no conversation contents or metrics. Enhanced measurement also records automatic interactions and their metadata after consent. Visits that decline analytics or block tracking will not appear.</div></div></main>}
